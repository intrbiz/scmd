...
class SCMD
{
    public:
        ...
        SCMD_STATUS writeHeader(byte command, uint16_t len);
        SCMD_STATUS writeDataChunk(byte *data, byte len);
        SCMD_STATUS readAck();
        SCMD_STATUS readCommand(byte *command, uint16_t *payloadLength, byte *payload, uint16_t maxPayloadLen);
        SCMD_STATUS readHeader(byte *header);
        SCMD_STATUS readDataChunk(byte *data);
        void setOnReceive(void (*callback)(byte, byte*, uint16_t));
    ...
    private:
        ...
        Stream *_debugTo;
        bool _debug;
        void (*callback)(byte, byte*, uint16_t);
